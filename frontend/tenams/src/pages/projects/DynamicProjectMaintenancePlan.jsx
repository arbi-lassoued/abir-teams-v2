...
import getProjectMaintenancePlanAPI from "../../api/projects/getProjectMaintenancePlanAPI"
...
const DynamicProjectMaintenancePlan = () => {
    ...
    // Load project and pm data
    const loadProjectData = React.useCallback(async () => {
        if (!projectId) return;

        try {
            setLoading(true);
            // In a real app, you'd fetch project details and pm
            const equipmentData = await getProjectMaintenancePlanAPI(projectId);
            setPm(equipmentData);
            ...
            setError(`Failed to load maintenance plan: ${error.message}`);
        } finally {
            setLoading(false);
        }
    }, [projectId]);
    ...
    //         setCsvFile(null);
    //         loadProjectData(); // Refresh pm list
    //     } catch (error) {
    ...
    }, [pm, searchTerm, filters]);

    // Pagination
    const paginatedPm = useMemo(() => {
        const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
        return filteredPm.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    }, [filteredPm, currentPage]);

    const totalPages = Math.ceil(filteredPm.length / ITEMS_PER_PAGE);

    if (loading) return <div className="flex justify-center items-center h-64">Loading project maintenance Plan...</div>;
        ...
        <div className="container mx-auto p-6 space-y-6">
            ...
            <Card className="bg-violet-50 border-violet-200">
                <CardContent className="p-4">
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-lg font-semibold text-violet-800">Planned Maintenance Summary</h2>
                            <p className="text-violet-600">{pm.length} Maintenance plan items</p>
                        </div>
                        <div className="text-right">
                            <p className="text-sm text-violet-600">Showing {filteredPm.length} filtered items</p>
                        </div>
                    </div>
                </CardContent>
            </Card>
                        ...
                        <table className="w-full">
                            <thead className="bg-[#0070EF] text-white">
                                <tr>
                                    ...
                                    <th className="p-4 text-left font-semibold">maint_description</th>
                                    <th className="p-4 text-left font-semibold">frequency</th>
                                    <th className="p-4 text-left font-semibold">Status</th>
                                    <th className="p-4 text-left font-semibold">Manpower</th>
                                ...
                                {paginatedPm.map((item) => (
                                    <tr key={item.id} className="hover:bg-violet-50">
                                        ...
                                        <td className="p-4">
                                            ...
                                            <div className="font-medium text-gray-900">{item.sub_equipment_class_code}</div>
                                        </td>
                                        <td className="p-4 text-gray-600">{item.maint_description}</td>
                                        <td className="p-4 text-gray-600">{item.frequency}</td>
                                        <td className="p-4">
                                            <div className="text-gray-600">Asset : {item.eq_status}</div>
                                            <div className=" text-gray-600">Plant : {item.plant_status}</div>
                                        </td>
                                        <td className="p-4 text-gray-600 justify-end">{item.manpower} Technician(s)</td>
                                        <td className="p-4 text-gray-600 justify-end">{item.active_time}</td>
                                        <td className="p-4 text-gray-600 justify-end">{item.scope}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
...
export default DynamicProjectMaintenancePlan; 